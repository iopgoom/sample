...
(function(w) {
    try {
        ...
        if (wp_uid) {
            ...
            if (is_wp_uid) {
                identity.setCookie('_wp_uid', '2-f725787c5fdfeda653b8936544ee8d01-s1596077358.9973|windows_10|chrome-pkbf7b', 365);
                return;
            }
        }
        ...
        if (size>3980 || len>45 || !is_wp_uid) {
            if (/^(http|https)$/.test(scheme)) {
                i = new Image();
                i.src = scheme + '://altg.widerplanet.com/delivery/info?dlid=&count=' + len + '&size=' + size + '&is_wp_uid=' + is_wp_uid + '&ti=27101&ga=' + ga + (is_wp_uid === 0 ? '&org_wp_uid=' + encodeURIComponent(wp_uid) : '');
            }
        } else {
            identity.setCookie('_wp_uid', '2-f725787c5fdfeda653b8936544ee8d01-s1596077358.9973|windows_10|chrome-pkbf7b', 365);
        }
